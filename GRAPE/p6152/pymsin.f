...
C...Local variables.
 ...
 1    DOUBLE PRECISION ATERM,TAN2T,THETA,DENOM
...
C...Coupling to W
      PARU(175)=PARU(174)
C...Coupling to h
C      PARU(176)=COS(2D0*AL)*COS(BE+AL)-2D0*SIN(2D0*AL)*SIN(BE+AL)
...
C...Coupling to H+
C...Define later
C      PARU(168)=-SBMA-COS(2D0*BE)*SAPB/2D0/(1D0-XW)
...
C...Coupling to A
C      PARU(177)=COS(2D0*BE)*COS(BE+AL)
...
C...Calculate decays to non-SUSY particles.
          ...
          DO 160 I1=0,100
            XLAM(I1)=0D0
  160     CONTINUE
          DO 180 I1=1,MDCY(KC,3)
            K1=MDCY(KC,2)+I1-1
            IF(IABS(KFDP(K1,1)).GT.KSUSY1.OR.
     &      IABS(KFDP(K1,2)).GT.KSUSY1) GOTO 180
            XLAM(I1)=WDTP(I1)
...
C...thanks to K. Matchev (FNAL)
        DO 185 IDC=MDCY(KC,2),MDCY(KC,2)+MDCY(KC,3)-1
          BRAT(IDC)=0D0
  185   CONTINUE
