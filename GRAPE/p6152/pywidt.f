...
C...Local arrays and saved variables.
     ...
     &WID2SV(3,2),WDTPP(0:200),WDTEP(0:200,0:5)
...
C...t -> ~g + ~t
            ...
            IF(SHR.GT.PMNCHI+PMSTOP) THEN
              ...
              PCM=SQRT((SH-(PMNCHI+PMSTOP)**2)*
     &        (SH-(PMNCHI-PMSTOP)**2))/(2D0*SHR)
              WDTP(I)=4D0/3D0*0.5D0*PYALPS(SH)*PCM*((FL**2+FR**2)*
     &        (SH+PMNCHI**2-PMSTOP**2)+PMNCHI*4D0*SHR*FL*FR)/SH
              IF(KFLR.GT.0) THEN
                WID2=WIDS(KFC1,2)*WIDS(KFC2,2)
              ELSE
                WID2=WIDS(KFC1,2)*WIDS(KFC2,3)
...
C...h0 (or H0, or A0):
     ...
     &    GOTO 260
          ...
          IF(I.LE.8) THEN
C...h0 -> q + qbar
            WDTP(I)=FAC*3D0*(PYMRUN(KFDP(IDC,1),SH)**2/SH)*
...
C...A0 behaves like beta, ho and H0 like beta**3.
          ...
          ELSEIF(I.LE.12) THEN
C...h0 -> l+ + l-
            WDTP(I)=FAC*RM1*SQRT(MAX(0D0,1D0-4D0*RM1))
C...A0 behaves like beta, ho and H0 like beta**3.
            IF(IHIGG.NE.3) WDTP(I)=WDTP(I)*(1D0-4D0*RM1)
...
C...R -> l+ + l'-
  ...
  330   CONTINUE
      ...
      ELSEIF(KFLA.EQ.51.OR.KFLA.EQ.53) THEN
...
C...pi+_tech
        ...
        DO 350 I=1,MDCY(KC,3)
          ...
          PM2=PMAS(PYCOMP(KFDP(IDC,2)),1)
          PM3=0D0
          IF(I.EQ.3) PM3=PMAS(PYCOMP(KFDP(IDC,3)),1)
          RM1=PM1**2/SH
          RM2=PM2**2/SH
          RM3=PM3**2/SH
...
C...pi_tech+ -> W b b~
          IF(I.EQ.3.AND.SHR.LT.PMAS(6,1)+PMAS(5,1)) THEN
            FCOF=3D0*RADC
            XMT2=PMAS(6,1)**2/SH
            ...
            CHECK = SQRT(RM1)
            T0 = (1D0-CHECK**2)*
     &      (XMT2*(6.*XMT2**2+3.*XMT2*RM1-4.*RM1**2)-
     &      (5.*XMT2**2+2.*XMT2*RM1-8.*RM1**2))/(4.*XMT2**2)
            T1 = (1D0-XMT2)*(RM1-XMT2)*((XMT2**2+XMT2*RM1+4.*RM1**2)
     &      -3.*XMT2**2*(XMT2+RM1))/(2.0*XMT2**3)
            T3 = RM1**2/XMT2**3*(3.0*XMT2-4.0*RM1+4.0*XMT2*RM1)
...
C...rho_tech0 -> W+ + pi_tech-.
     ...
     &      (1D0-PARP(141)**2)/4D0/XW/24D0/PARJ(173)**2*SHR**3
...
C...rho_tech0 -> pi_tech+ + pi_tech-.
     ...
     &      SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3
            WID2=WIDS(52,1)
          ELSEIF(I.EQ.5) THEN
C...rho_tech0 -> gamma + pi_tech0
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (2D0*PARP(143)-1D0)**2*(1D0-PARP(141)**2)/24D0/PARJ(172)**2*
     &      SHR**3
            WID2=WIDS(51,2)
          ELSEIF(I.EQ.6) THEN
C...rho_tech0 -> gamma + pi_tech0'
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (1D0-PARJ(174)**2)/24D0/PARJ(172)**2*SHR**3
            WID2=WIDS(53,2)
          ELSEIF(I.EQ.7) THEN
C...rho_tech0 -> Z0 + pi_tech0
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (2D0*PARP(143)-1D0)**2*(1D0-PARP(141)**2)/24D0/PARJ(172)**2*
     &      XW/XW1*SHR**3
...
C...rho_tech0 -> Z0 + pi_tech0'
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (1D0-PARJ(174)**2)/24D0/PARJ(172)**2*(1D0-2D0*XW)**2/4D0/
     &      XW/XW1*SHR**3
            WID2=WIDS(23,2)*WIDS(53,2)
          ELSE
C...rho_tech0 -> f + fbar.
            WID2=1D0
            IF(I.LE.16) THEN
...
C...rho_tech+ -> pi_tech+ + Z0.
     ...
     &      (1D0-PARP(141)**2)/4D0/XW/XW1/24D0/PARJ(173)**2*SHR**3+
...
C...rho_tech+ -> W+ + pi_tech0'
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (1D0-PARJ(174)**2)/4D0/XW/24D0/PARJ(172)**2*SHR**3
            IF(KFLR.GT.0) THEN
              WID2=WIDS(24,2)*WIDS(53,2)
            ELSE
              WID2=WIDS(24,3)*WIDS(53,2)
            ENDIF
          ELSE
C...rho_tech+ -> f + fbar'.
            IA=I-6
...
C...omega_tech0 -> gamma + pi_tech0.
            WDTP(I)=AEM/24D0/PARJ(172)**2*(1D0-PARP(141)**2)*
     &      SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*SHR**3
            WID2=WIDS(51,2)
          ELSEIF(I.EQ.2) THEN
C...omega_tech0 -> Z0 + pi_tech0 
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (1D0-PARP(141)**2)/24D0/PARJ(172)**2*(1D0-2D0*XW)**2/4D0/
     &      XW/XW1*SHR**3
            WID2=WIDS(23,2)*WIDS(51,2)
          ELSEIF(I.EQ.3) THEN
C...omega_tech0 -> gamma + pi_tech0'
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (2D0*PARP(143)-1D0)**2*(1D0-PARJ(174)**2)/24D0/PARJ(172)**2*
     &      SHR**3
            WID2=WIDS(53,2)
          ELSEIF(I.EQ.4) THEN
C...omega_tech0 -> Z0 + pi_tech0'
     ...
     &      XW/XW1*SHR**3
            WID2=WIDS(23,2)*WIDS(51,2)
          ELSEIF(I.EQ.5) THEN
C...omega_tech0 -> W+ + pi_tech-
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (1D0-PARP(141)**2)/4D0/XW/24D0/PARJ(172)**2*SHR**3+
     &      FAC*PARP(141)**2*(1D0-PARP(141)**2)*PARJ(175)**2*
     &      SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3
            WID2=WIDS(24,2)*WIDS(52,3)
          ELSEIF(I.EQ.6) THEN
C...omega_tech0 -> pi_tech+ + W-
            WDTP(I)=AEM*SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3*
     &      (1D0-PARP(141)**2)/4D0/XW/24D0/PARJ(172)**2*SHR**3+
     &      FAC*PARP(141)**2*(1D0-PARP(141)**2)*PARJ(175)**2*
     &      SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3
            WID2=WIDS(24,3)*WIDS(52,2)
          ELSEIF(I.EQ.7) THEN
C...omega_tech0 -> W+ + W-.
            WDTP(I)=FAC*PARP(141)**4*PARJ(175)**2*
     &      SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3
            WID2=WIDS(24,1)
          ELSEIF(I.EQ.8) THEN
C...omega_tech0 -> pi_tech+ + pi_tech-.
            WDTP(I)=FAC*(1D0-PARP(141)**2)**2*PARJ(175)**2*
     &      SQRT(MAX(0D0,(1D0-RM1-RM2)**2-4D0*RM1*RM2))**3
            WID2=WIDS(52,1)
          ELSE
C...omega_tech0 -> f + fbar.
  ...
  380   CONTINUE
      ...
      ELSEIF(KFLA.EQ.61) THEN
C...H_L++/--:
        FAC=(1D0/(8D0*PARU(1)))*SHR
        DO 372 I=1,MDCY(KC,3)
          IDC=I+MDCY(KC,2)-1
          IF(MDME(IDC,1).LT.0) GOTO 372
          RM1=PMAS(IABS(KFDP(IDC,1)),1)**2/SH
          RM2=PMAS(IABS(KFDP(IDC,2)),1)**2/SH
          IF(SQRT(RM1)+SQRT(RM2).GT.1D0) GOTO 372
          WID2=1D0
          IF(I.LE.6) THEN
C...H_L++/-- -> l+/- + l'+/-
            FCOF=PARP(180+3*((IABS(KFDP(IDC,1))-11)/2)+
     &      (IABS(KFDP(IDC,2))-9)/2)**2
C***Should be factor 4 below ???
...
C...H_L++/-- -> W_L+/- + W_L+/-
  ...
  372   CONTINUE
      ...
      ELSEIF(KFLA.EQ.62) THEN
C...H_R++/--:
        FAC=(1D0/(8D0*PARU(1)))*SHR
        DO 373 I=1,MDCY(KC,3)
          IDC=I+MDCY(KC,2)-1
          IF(MDME(IDC,1).LT.0) GOTO 373
          RM1=PMAS(IABS(KFDP(IDC,1)),1)**2/SH
          RM2=PMAS(IABS(KFDP(IDC,2)),1)**2/SH
          IF(SQRT(RM1)+SQRT(RM2).GT.1D0) GOTO 373
          WID2=1D0
          IF(I.LE.6) THEN
C...H_R++/-- -> l+/- + l'+/-
            FCOF=PARP(180+3*((IABS(KFDP(IDC,1))-11)/2)+
...
C...H_R++/-- -> W_R+/- + W_R+/-
 ...
 373   CONTINUE
      ...
      ELSEIF(KFLA.EQ.63) THEN
C...W_R+/-:
        FAC=(AEM/(24D0*XW))*SHR
        DO 374 I=1,MDCY(KC,3)
          IDC=I+MDCY(KC,2)-1
          IF(MDME(IDC,1).LT.0) GOTO 374
          RM1=PMAS(IABS(KFDP(IDC,1)),1)**2/SH
          RM2=PMAS(IABS(KFDP(IDC,2)),1)**2/SH
          IF(SQRT(RM1)+SQRT(RM2).GT.1D0) GOTO 374
          WID2=1D0
          IF(I.LE.9) THEN
C...W_R+/- -> q + qbar'
