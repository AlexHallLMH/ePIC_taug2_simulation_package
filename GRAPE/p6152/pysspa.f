...
C...Read out basic information; set global Q^2 scale.
      ...
      MECOR=0
      IF(MSTP(68).EQ.1.AND.(ISUB.EQ.1.OR.ISUB.EQ.2.OR.
     &ISUB.EQ.141.OR.ISUB.EQ.142.OR.ISUB.EQ.144)) MECOR=1      
      FCQ2MX=1D0
...
C...q -> q, g -> q.
      IF(IABS(KFLB).LE.10) THEN
        ...
        IF(MECOR.EQ.1.AND.(N.EQ.NS+1.OR.N.EQ.NS+2)) 
     &  WTAPC(21)=3D0*WTAPC(21)
C...f -> f, gamma -> f.
...
C...Remove kinematically impossible branchings.
      ...
      IF(MSTP(68).GE.0.AND.UHAT.GT.0D0) GOTO 210

C...Matrix-element corrections for s-channel resonance production.
      IF(MECOR.EQ.1.AND.(N.EQ.NS+1.OR.N.EQ.NS+2)) THEN
        SHAT=DSH/Z
        THAT=-Q2B
        IF(IABS(KFLA).LE.20.AND.IABS(KFLB).LE.20) THEN
          RMEPS=(THAT**2+UHAT**2+2D0*DSH*SHAT)/(SHAT**2+DSH**2)
          WTZ=WTZ*RMEPS
        ELSEIF((KFLA.EQ.21.OR.KFLA.EQ.22).AND.IABS(KFLB).LE.20) THEN
          RMEPS=(SHAT**2+UHAT**2+2D0*DSH*THAT)/((SHAT-DSH)**2+DSH**2)
          WTZ=WTZ*RMEPS/3D0
        ENDIF 
      ENDIF
