...
while not reader.failed():
    ...
    for p in particles:
        ...
        if p.production_vertex:
            ...
            old_vtx_id = id(old_vtx)
            if old_vtx_id not in vertex_map:
                new_vtx = pyhepmc.GenVertex(old_vtx.position)
                vertex_map[old_vtx_id] = new_vtx
                vertex_id_map[old_vtx_id] = old_vtx
                new_event.add_vertex(new_vtx)
            vertex_map[old_vtx_id].add_particle_out(new_p)
        else:
            new_event.add_particle(new_p)  # incoming particle

    for old_vtx_id, new_vtx in vertex_map.items():
        old_vtx = vertex_id_map[old_vtx_id]
