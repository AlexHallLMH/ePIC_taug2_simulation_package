...
*     AAM !=0 , AM != 0
*-----------------------------------------------------------------------
      SUBROUTINE FFVMM1(LIND,II,IO,AL,AR,CC,C,Q1,Q2,P1,P2,Q,AALL,AM1,AM2)
      IMPLICIT REAL*8(A-H,O-Z)
      PARAMETER (ONE = 1.0D0)
      
C----- Use the common block to get a_tau from CONFIG_READER
      DOUBLE PRECISION a_tau
      COMMON /TAU_CONFIG/ a_tau

      DOUBLE PRECISION DeltaALL11R, DeltaALL11I
      DOUBLE PRECISION DeltaALL22R, DeltaALL22I
      DOUBLE PRECISION DeltaALL12R, DeltaALL12I
      DOUBLE PRECISION DeltaALL21R, DeltaALL21I


C----- ALEX's declarations 2025 (declarations must be made up here)
      DOUBLE PRECISION SUM(4)
      DOUBLE PRECISION DeltaGamma(4)
...
C----- Add the anomalous magnetic moment contribution.
      ...
      DO mu = 1, 4
         DeltaGamma(mu) = - a_tau / (2.0D0 * AM1) * SUM(mu)
      END DO

      

        IF( AM1 .GT. 1.0D0 .AND. AM2 .GT. 1.0D0 ) THEN
           AALL(LIND,J1,K1) = AL * (1.0D0 + a_tau) * DCMPLX(ALL11R, -ALL11I)
     &                 + (AR * R * RR * C(2) * CC(2) * (1.0D0 + a_tau) * DCMPLX(ALL22R, ALL22I) )
     &                 + DCMPLX(DeltaGamma(1), DeltaGamma(2))
           AALL(LIND,J2,K2) = AR * DCMPLX((1.0D0 + a_tau)*ALL11R, (1.0D0 + a_tau)*ALL11I) 
     &                 + (AL * R * RR * C(1) * CC(1) * DCMPLX((1.0D0 + a_tau)*ALL22R, -(1.0D0 + a_tau)*ALL22I)) 
     &                 + DCMPLX(DeltaGamma(3), DeltaGamma(4))
           AALL(LIND,J1,K2) = -(AR * RR * CC(2) * DCMPLX((1.0D0 + a_tau)*ALL21R, (1.0D0 + a_tau)*ALL21I)) 
     &                 -(AL * R * C(1)  * DCMPLX((1.0D0 + a_tau)*ALL12R, -(1.0D0 + a_tau)*ALL12I)) 
     &                 + DCMPLX(DeltaGamma_12_R, DeltaGamma_12_I)
           AALL(LIND,J2,K1) = -(AL * RR * CC(1) * DCMPLX((1.0D0 + a_tau)*ALL21R, -(1.0D0 + a_tau)*ALL21I)) 
     &                 -(AR * R * C(2)  * DCMPLX((1.0D0 + a_tau)*ALL12R, (1.0D0 + a_tau)*ALL12I)) 
     &                 + DCMPLX(DeltaGamma_21_R, DeltaGamma_21_I)
