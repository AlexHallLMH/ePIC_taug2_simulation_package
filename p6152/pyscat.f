...
C...f + fbar -> ~e_R + ~e_Lbar
          KCSG=1
          IF(MINT(2).EQ.2) KCSG=-1
          MINT(21)=ISIGN(KSUSY1+11,KCSG)
...
C...f + fbar -> ~mu_L + ~mu_Rbar
          KCSG=1
          IF(MINT(2).EQ.2) KCSG=-1
          MINT(21)=ISIGN(KSUSY1+13,KCSG)
          MINT(22)=-ISIGN(KSUSY2+13,KCSG)
...
C...f + fbar -> ~tau_1 + ~tau_2bar
          KCSG=1
          IF(MINT(2).EQ.2) KCSG=-1
          MINT(21)=ISIGN(KSUSY1+15,KCSG)
          MINT(22)=-ISIGN(KSUSY2+15,KCSG)
          ...
          DO 432 JT=1,2
            ...
            IF(IA.LE.10) THEN
              ...
              DO 422 J=1,MSTP(1)
                ...
                IPM=(5-ISIGN(1,I))/2
                IDC=J+MDCY(IA,2)+2
                IF(MDME(IDC,1).NE.1.AND.MDME(IDC,1).NE.IPM) GOTO 422
                MINT(20+JT)=ISIGN(IB,I)
                RVCKM=RVCKM-VCKM((IA+1)/2,(IB+1)/2)
                IF(RVCKM.LE.0D0) GOTO 432
  422         CONTINUE
            ELSE
              IB=2*((IA+1)/2)-1+MOD(IA,2)
              MINT(20+JT)=ISIGN(IB,I)
...
C...f + fbar -> neutral neutral
        ELSEIF(ISUB.LE.367) THEN
          MINT(21)=KFPR(ISUB,1)
          MINT(22)=KFPR(ISUB,2)
C...f + fbar' -> charged neutral
        ELSEIF(ISUB.EQ.374.OR.ISUB.EQ.375) THEN
          ...
          KCH2=KCHG(IABS(MINT(16)),1)*ISIGN(1,MINT(16))
          IF(MINT(15)*(KCH1+KCH2).LT.0) JS=2
c         MINT(20+JS)=ISIGN(KFPR(ISUB,IC),KCH1+KCH2)
c         MINT(23-JS)=KFPR(ISUB,IN)
