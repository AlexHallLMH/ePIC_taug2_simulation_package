...
*     AAM =0 , AM = 0
*-----------------------------------------------------------------------
      SUBROUTINE FFVMM9(LIND,II,IO,AL,AR,CC,C,Q1,Q2,P1,P2,Q,AALL,AM1,AM2)
...
C         of (i/2)[γ^μ,γ^ν]q_ν onto the corresponding helicity basis.
      ...
      IF( AM1 .GT. 1.0D0 .OR. AM2 .GT. 1.0D0 ) THEN
          ...
          AALL(LIND,1,K1) = AALL(LIND,1,K1)
     &                 + a_tau * DCMPLX(DeltaALL11R, DeltaALL11I)
          AALL(LIND,2,K2) = AALL(LIND,2,K2) 
     &                 + a_tau * DCMPLX(DeltaALL22R, DeltaALL22I)
          AALL(LIND,1,K2) = AALL(LIND,1,K2) 
     &                 + a_tau * DCMPLX(DeltaALL21R, DeltaALL21I)
          AALL(LIND,2,K1) = AALL(LIND,2,K1) 
